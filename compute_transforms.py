...
ROOT = r"c:\Users\Xeon\Documents\GitHub\Digital-Banker-screen"

# ---------- 1. Stage size ----------
bg = Image.open(f"{ROOT}/BACKGROUND.png")
STAGE_W, STAGE_H = bg.size
print("STAGE:", STAGE_W, STAGE_H)

# ---------- 2. Template zone detection ----------
tpl = Image.open(f"{ROOT}/ШАБЛОН v1.png").convert("RGB")
assert tpl.size == (STAGE_W, STAGE_H)

a = np.array(tpl).astype(np.int16)
mx = np.max(a, axis=2)
mn = np.min(a, axis=2)
sat = (mx - mn) / np.clip(mx, 1, None)
mask = (sat < 0.22) & (mx > 140)
...
# ---------- 2b. Manual per-arch width correction (1, 2, 4, 5 only) ----------
# Symmetric widening of the destination quad around its existing center: the left edge
# (TL/BL) moves left by WIDEN_HALF_PX[id], the right edge (TR/BR) moves right by the same
# amount, Y untouched. Equal px on both sides keeps each arch's horizontal center exactly
# fixed (0.0px drift) while growing width, and each value is bounded by the tighter of the
# two measured real photographic margins to the adjacent fluted pilaster in BACKGROUND.png
# (arch1: ~17px left vs ~2px right; arch2: ~10px left vs ~7px right; arch4: ~3px left vs
# ~6px right; arch5: ~11px left vs ~3px right), minus a small safety buffer, so no arch
# overlaps a pilaster or a neighbor. Arches 3 (Move Money) and 6 (Record Library) are
# intentionally excluded -- they stay exactly at the auto-detected template zone.
WIDEN_HALF_PX = {1: 3, 2: 5, 4: 2, 5: 2}
for arch_id, half in WIDEN_HALF_PX.items():
    panel_corners[arch_id]["TL"][0] -= half
    panel_corners[arch_id]["BL"][0] -= half
    panel_corners[arch_id]["TR"][0] += half
    panel_corners[arch_id]["BR"][0] += half

# ---------- 3. FULL bbox + MAIN ARCH BBOX (largest connected alpha component) ----------
arches = {}
for i in range(1, 7):
    im = Image.open(f"{ROOT}/{i}.png")
    arr = np.array(im)
    alpha = arr[:, :, 3]

    # FULL bbox: alpha>0 across whole image (title+arch combined) -- unchanged rule, used for cropping/export only
    ...
    cropped.save(f"{ROOT}/cropped/{i}.png")

    # MAIN ARCH BBOX: connected-components on the full alpha mask, take the single
    # largest-area component (the arch body). Title text and thin stray/glow alpha
    # slivers are separate, much smaller components and are ignored here.
...
for i in range(1, 7):
    ...
    # main-arch X bounds, translated into cropped(full-bbox-local) coords
    lx0 = mb["x"] - fb["x"]
    lx1 = mb["x"] + mb["w"] - fb["x"]
    full_h = fb["h"]

    # source quad: X from MAIN_ARCH_BBOX, Y spans the *entire* full (title+arch) image
    # height, 0..full_h, so the vertical placement/scale reflects the whole
    # composition (title+arch), not just the arch body.
    ...
    # visible-content fill check: project the MAIN_ARCH_BBOX left/right edges (at
    # mid-height) and compare to the panel's left/right edges at the same relative height
...
result = {
    "meta": {
        "coordinate_system_note": f"Stage = BACKGROUND.png {STAGE_W}x{STAGE_H}.",
        "arch_bbox_method": "Width of the source quad comes from the largest CONNECTED COMPONENT of the alpha mask (the actual arch body polygon, ~563px) -- not the raw alpha>0 bbox, which is contaminated by thin disconnected stray-alpha slivers near x~662-672 and by the title text. Height of the source quad spans the full (title+arch) image, 0..full_h, so the title rides the same projective extrapolation above the arch instead of being cropped out of the mapping.",
        "alpha_threshold": "alpha > 0 (строго по ТЗ) for full_bbox export/crop. MAIN_ARCH_BBOX uses connectedComponentsWithStats over the same alpha>0 mask, keeping only the largest-area component.",
        "homography_method": "cv2.getPerspectiveTransform(src=[ (mainArchX0,0),(mainArchX1,0),(mainArchX1,full_h),(mainArchX0,full_h) ], dst=[TL,TR,BR,BL] of template zone) -> CSS matrix3d, applied to the FULL (title+arch) cropped image element.",
    },
    "stage": {"width": STAGE_W, "height": STAGE_H},
    "arches": arches,
}

with open(f"{ROOT}/arch_transforms.json", "w", encoding="utf-8") as f:
